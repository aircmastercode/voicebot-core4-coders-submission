...
class NLPPipeline:
    ...
    def process_input(self, text: str, session_id: Optional[str] = None, history: Optional[List[Dict[str, str]]] = None, 
        ...
        # Run the async WebSocket call in the event loop
        if stream_handler:
            # For streaming mode, we'll get the session ID as the return value
            session_id = self.loop.run_until_complete(self.ws_client.send_message(payload, stream_handler))
            return {"session_id": session_id} if session_id else None
        else:
            # For non-streaming mode, we'll get the complete response
            response = self.loop.run_until_complete(self.ws_client.send_message(payload))
            ...
            if response:
                ...
                # Generate audio if needed and not already generated by WebSocketClient
